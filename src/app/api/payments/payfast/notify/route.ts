...
async function sendOrderNotifications(order: any) {
  try {
    ...
    // ── WhatsApp: customer confirmation (always send directly) ───────────────
    ...
    if (agentSecret) {
      fetch(`${engageUrl}/api/v1/agent/payment-confirmed`, {
        method: 'POST',
      ...
      }).then(r => console.log('[ITN] Engage Africa notified:', r.status))
        .catch(err => console.warn('[ITN] Engage Africa notify failed (non-critical):', err?.message))
    } else {
      console.warn('[ITN] AGENT_API_SECRET not set — skipping Engage Africa notify (dispatch already sent above)')
    }
